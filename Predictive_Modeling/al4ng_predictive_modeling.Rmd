...
Code modified from the [**course webpage source file**](https://github.com/bioconnector/workshops/blob/master/r-predictive-modeling.Rmd)
...
# Set the number of significant digits (default is 7)
options(digits = 3)

# Set the maximum number of entries printed by print() or show() methods (default is 99999)
options(max.print = 200)

# Track ex numbers w/ hidden var. 
#   Increment each ex: `r .ex``r .ex=.ex+1`
.ex <- 1 
...
The data we're using here is from the 2013 outbreak of [influenza A H7N9 in China](https://en.wikipedia.org/wiki/Influenza_A_virus_subtype_H7N9), analyzed by Kucharski _et al._, published in 2014.
...
There are two datasets available to download on the [data](http://bims.fun/data.html) page: 
...
Let's use ggplot2 to take a look at the data. Refer back to the [ggplot2 class](http://bims.fun/r-viz-gapminder.html) if you need a refresher.
...
flu %>%
    ...
    select(-gender) %>%
    mutate(hospital = !is.na(date_hospitalization)) %>%
    mutate(days_to_hospital = 
               as.numeric(date_hospitalization - date_onset)) %>%
    mutate(days_to_outcome  = 
               as.numeric(date_outcome - date_onset)) %>%
    mutate(early_outcome = 
               date_outcome < median(date_outcome, na.rm = TRUE)) %>%
...
![_The caret model training algorithm. Image from the caret paper._](http://bims.fun/img/caret-pseudocode.png)
...
![_Bootstrapping schematic. Image from Max Kuhn's 2013 useR! talk._](http://bims.fun/img/caret-bootstrap.png)
...
![_Schematic view of the analyst-in-the-loop approach to forecasting at scale, which best makes use of human and automated tasks. Image from the Prophet preprint noted above._](http://bims.fun/img/prophet.png)
